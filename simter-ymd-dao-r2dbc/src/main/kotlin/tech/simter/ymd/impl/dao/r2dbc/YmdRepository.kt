 ...
 * @author RJ
 */
interface YmdRepository : Repository<YmdPo, String>, CustomizedYmdRepository {
  ...
  @Query("select distinct year from $TABLE_YMD where type = ?1 order by year desc")
  fun findYears(type: String): Flux<Int>

  @Query("select distinct month from $TABLE_YMD where type = ?1 and year = ?2 order by month desc")
  fun findMonths(type: String, year: Int): Flux<Int>

  @Query("select distinct day from $TABLE_YMD where type = ?1 and  year = ?2 and month = ?3 order by day desc")
  ...
  */
  ...
  fun findById(id: String): Mono<YmdPo>
